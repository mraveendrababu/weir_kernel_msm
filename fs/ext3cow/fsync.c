...
int ext3cow_sync_file(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	trace_ext3cow_sync_file_enter(file, datasync);
	...
	J_ASSERT(ext3cow_journal_current_handle() == NULL);
...
out:
	trace_ext3cow_sync_file_exit(inode, ret);
	return ret;
}
