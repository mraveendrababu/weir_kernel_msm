 ...
 * when a file system is mounted (see ext3cow_fill_super).
 */
 ...
 */
static void ext3cow_get_group_no_and_offset(struct super_block *sb,
	ext3cow_fsblk_t blocknr, unsigned long *blockgrpp, ext3cow_grpblk_t *offsetp)
{
	struct ext3cow_super_block *es = EXT3COW_SB(sb)->s_es;

	blocknr = blocknr - le32_to_cpu(es->s_first_data_block);
	if (offsetp)
		*offsetp = blocknr % EXT3COW_BLOCKS_PER_GROUP(sb);
	if (blockgrpp)
		*blockgrpp = blocknr / EXT3COW_BLOCKS_PER_GROUP(sb);
}
...
static int ext3cow_valid_block_bitmap(struct super_block *sb,
					struct ext3cow_group_desc *desc,
					unsigned int block_group,
					struct buffer_head *bh)
{
	ext3cow_grpblk_t offset;
	ext3cow_grpblk_t next_zero_bit;
	ext3cow_fsblk_t bitmap_blk;
	ext3cow_fsblk_t group_first_block;

	group_first_block = ext3cow_group_first_block_no(sb, block_group);

	/* check whether block bitmap block number is set */
	bitmap_blk = le32_to_cpu(desc->bg_block_bitmap);
	offset = bitmap_blk - group_first_block;
	if (!ext3cow_test_bit(offset, bh->b_data))
		/* bad block bitmap */
		goto err_out;
	...
	offset = bitmap_blk - group_first_block;
	next_zero_bit = ext3cow_find_next_zero_bit(bh->b_data,
				offset + EXT3COW_SB(sb)->s_itb_per_group,
				offset);
	if (next_zero_bit >= offset + EXT3COW_SB(sb)->s_itb_per_group)
...
read_block_bitmap(struct super_block *sb, unsigned int block_group)
{
	...
	if (!desc)
		return NULL;
	trace_ext3cow_read_block_bitmap(sb, block_group);
	bitmap_blk = le32_to_cpu(desc->bg_block_bitmap);
	...
	ext3cow_valid_block_bitmap(sb, desc, block_group, bh);
	/*
		    ...
		    struct ext3cow_reserve_window_node *rsv)
{
	...
	trace_ext3cow_rsv_window_add(sb, rsv);
	while (*p)
	{
		parent = *p;
...
void ext3cow_discard_reservation(struct inode *inode)
{
	...
	if (!rsv_is_empty(&rsv->rsv_window)) {
		spin_lock(rsv_lock);
		if (!rsv_is_empty(&rsv->rsv_window)) {
			trace_ext3cow_discard_reservation(inode, rsv);
			...
			ext3cow_fsblk_t block, unsigned long count)
{
	...
	trace_ext3cow_free_blocks(inode, block, count);
		...
		unsigned int group, struct buffer_head *bitmap_bh)
{
	...
	trace_ext3cow_alloc_new_reservation(sb, start_block);
...
retry:
	...
	if (start_block >= my_rsv->rsv_start &&
		...
		trace_ext3cow_reserved(sb, start_block, my_rsv);
		return 0;		/* success */
	}
	/*
 ...
 */
static int ext3cow_has_free_blocks(struct ext3cow_sb_info *sbi, int use_reservation)
{
...
{
	if (!ext3cow_has_free_blocks(EXT3COW_SB(sb), 0) || (*retries)++ > 3)
		return 0;
			...
			ext3cow_fsblk_t goal, unsigned long *count, int *errp)
{
	...
	*errp = -ENOSPC;
	...
	trace_ext3cow_request_blocks(inode, goal, num);
	...
	if (!ext3cow_has_free_blocks(sbi, IS_NOQUOTA(inode))) {
		*errp = -ENOSPC;
		goto out;
	}
...
allocated:
	...
	*errp = 0;
	...
	trace_ext3cow_allocate_blocks(inode, goal, num,
...
{
	return ext3cow_bg_has_super(sb, group) ? EXT3COW_SB(sb)->s_gdb_count : 0;
}
...
/**
 * ext3cow_trim_all_free -- function to trim all free space in alloc. group
 ...
 *
 * ext3cow_trim_all_free walks through group's block bitmap searching for free
 * blocks. When the free block is found, it tries to allocate this block and
 * consequent free block to get the biggest free extent possible, until it
 * reaches any used block. Then issue a TRIM command on this extent and free
 * the extent in the block bitmap. This is done until whole group is scanned.
 */
static ext3cow_grpblk_t ext3cow_trim_all_free(struct super_block *sb,
					unsigned int group,
					ext3cow_grpblk_t start, ext3cow_grpblk_t max,
					ext3cow_grpblk_t minblocks)
{
	handle_t *handle;
	ext3cow_grpblk_t next, free_blocks, bit, freed, count = 0;
	ext3cow_fsblk_t discard_block;
	struct ext3cow_sb_info *sbi;
	 ...
	 */
	handle = ext3cow_journal_start_sb(sb, 2);
	if (IS_ERR(handle))
	...
	gdp = ext3cow_get_group_desc(sb, group, &gdp_bh);
	if (!gdp) {
		err = -EIO;
		goto err_out;
	}
	...
	BUFFER_TRACE(gdp_bh, "get_write_access");
	err = ext3cow_journal_get_write_access(handle, gdp_bh);
	if (err)
		goto err_out;
	...
	while (start <= max) {
		...
		discard_block = (ext3cow_fsblk_t)start +
				ext3cow_group_first_block_no(sb, group);
		...
		trace_ext3cow_discard_blocks(sb, discard_block, next - start);
...
free_extent:
		...
		for (bit = start; bit < next; bit++) {
			BUFFER_TRACE(bitmap_bh, "clear bit");
			if (!ext3cow_clear_bit_atomic(sb_bgl_lock(sbi, group),
		...
		if (err < 0) {
			if (err != -EOPNOTSUPP)
				ext3cow_warning(sb, __func__, "Discard command "
					     "returned error %d\n", err);
			break;
		}
	...
	/* We dirtied the bitmap block */
	BUFFER_TRACE(bitmap_bh, "dirtied bitmap block");
	ret = ext3cow_journal_dirty_metadata(handle, bitmap_bh);
	if (!err)
		err = ret;

	/* And the group descriptor block */
	BUFFER_TRACE(gdp_bh, "dirtied group descriptor block");
	ret = ext3cow_journal_dirty_metadata(handle, gdp_bh);
	if (!err)
		err = ret;

	ext3cow_debug("trimmed %d blocks in the group %d\n",
		count, group);
...
/**
 * ext3cow_trim_fs() -- trim ioctl handle function
 * @sb:			superblock for filesystem
 * @start:		First Byte to trim
 * @len:		number of Bytes to trim from start
 * @minlen:		minimum extent length in Bytes
 *
 * ext3cow_trim_fs goes through all allocation groups containing Bytes from
 * start to start+len. For each such a group ext3cow_trim_all_free function
 * is invoked to trim all free space.
 */
int ext3cow_trim_fs(struct super_block *sb, struct fstrim_range *range)
{
	ext3cow_grpblk_t last_block, first_block;
	unsigned long group, first_group, last_group;
	struct ext3cow_group_desc *gdp;
	struct ext3cow_super_block *es = EXT3COW_SB(sb)->s_es;
	uint64_t start, minlen, end, trimmed = 0;
	ext3cow_fsblk_t first_data_blk =
			le32_to_cpu(EXT3COW_SB(sb)->s_es->s_first_data_block);
	ext3cow_fsblk_t max_blks = le32_to_cpu(es->s_blocks_count);
	int ret = 0;
	...
	if (unlikely(minlen > EXT3COW_BLOCKS_PER_GROUP(sb)) ||
	...
	ext3cow_get_group_no_and_offset(sb, (ext3cow_fsblk_t) start,
				     &first_group, &first_block);
	ext3cow_get_group_no_and_offset(sb, (ext3cow_fsblk_t) end,
				     &last_group, &last_block);

	/* end now represents the last block to discard in this group */
	end = EXT3COW_BLOCKS_PER_GROUP(sb) - 1;

	for (group = first_group; group <= last_group; group++) {
		gdp = ext3cow_get_group_desc(sb, group, NULL);
		if (!gdp)
			break;

		/*
		 * For all the groups except the last one, last block will
		 * always be EXT3COW_BLOCKS_PER_GROUP(sb)-1, so we only need to
		 * change it for the last group, note that last_block is
		 * already computed earlier by ext3cow_get_group_no_and_offset()
		 */
		if (group == last_group)
			end = last_block;

		if (le16_to_cpu(gdp->bg_free_blocks_count) >= minlen) {
			ret = ext3cow_trim_all_free(sb, group, first_block,
						 end, minlen);
			if (ret < 0)
				break;
			trimmed += ret;
