...
void ext3cow_free_inode (handle_t *handle, struct inode * inode)
{
	...
	trace_ext3cow_free_inode(inode);
...
static int find_group_orlov(struct super_block *sb, struct inode *parent)
{
	...
	if ((parent == sb->s_root->d_inode) ||
		...
		for (i = 0; i < ngroups; i++) {
			...
			desc = ext3cow_get_group_desc (sb, group, NULL);
 ...
 */
struct inode *ext3cow_new_inode(handle_t *handle, struct inode * dir,
			     const struct qstr *qstr, umode_t mode)
{
	...
	sb = dir->i_sb;
	trace_ext3cow_request_inode(dir, mode);
	inode = new_inode(sb);
	if (!inode)
...
got:
	...
	ei->i_flags =
		ext3cow_mask_flags(mode, EXT3COW_I(dir)->i_flags & EXT3COW_FL_INHERITED);
	...
	ei->i_state_flags = 0;
	ext3cow_set_inode_state(inode, EXT3COW_STATE_NEW);

	/* See comment in ext3cow_iget for explanation */
	if (ino >= EXT3COW_FIRST_INO(sb) + 1 &&
	    EXT3COW_INODE_SIZE(sb) > EXT3COW_GOOD_OLD_INODE_SIZE) {
		ei->i_extra_isize =
			sizeof(struct ext3cow_inode) - EXT3COW_GOOD_OLD_INODE_SIZE;
	} else {
		ei->i_extra_isize = 0;
	}
	...
	err = ext3cow_init_security(handle, inode, dir, qstr);
	if (err)
		goto fail_free_drop;
	...
	trace_ext3cow_allocate_inode(inode, dir, mode);
	goto really_out;
fail:
	ext3cow_std_error(sb, err);
out:
...
struct inode *ext3cow_orphan_get(struct super_block *sb, unsigned long ino)
{
	 ...
	 */
	if (inode->i_nlink && !ext3cow_can_truncate(inode))
		goto bad_orphan;
