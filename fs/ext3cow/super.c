...
#ifdef CONFIG_EXT3COW_DEFAULTS_TO_ORDERED
  #define EXT3COW_MOUNT_DEFAULT_DATA_MODE EXT3COW_MOUNT_ORDERED_DATA
#else
  #define EXT3COW_MOUNT_DEFAULT_DATA_MODE EXT3COW_MOUNT_WRITEBACK_DATA
#endif
...
static int ext3cow_create_journal(struct super_block *, struct ext3cow_super_block *,
			       unsigned int);
static int ext3cow_commit_super(struct super_block *sb,
...
static int ext3cow_unfreeze(struct super_block *sb);
static int ext3cow_freeze(struct super_block *sb);
		...
		struct buffer_head *bh, handle_t *handle, int err)
{
	...
	printk(KERN_ERR "EXT3COW-fs: %s: aborting transaction: %s in %s\n",
		caller, errstr, err_fn);

	journal_abort_handle(handle);
}

void ext3cow_msg(struct super_block *sb, const char *prefix,
		const char *fmt, ...)
{
...
static void ext3cow_handle_error(struct super_block *sb)
{
	...
	if (!test_opt (sb, ERRORS_CONT)) {
		...
		set_opt(EXT3COW_SB(sb)->s_mount_opt, ABORT);
		if (journal)
			journal_abort(journal, -EIO);
	}
	if (test_opt (sb, ERRORS_RO)) {
		ext3cow_msg(sb, KERN_CRIT,
			"error: remounting filesystem read-only");
		sb->s_flags |= MS_RDONLY;
	}
	ext3cow_commit_super(sb, es, 1);
	if (test_opt(sb, ERRORS_PANIC))
		panic("EXT3COW-fs (%s): panic forced after error\n",
			sb->s_id);
}
		...
		const char *fmt, ...)
{
	...
	printk(KERN_CRIT "EXT3COW-fs error (device %s): %s: %pV\n",
		       ...
		       int errno)
{
	...
	ext3cow_msg(sb, KERN_CRIT, "error in %s: %s", function, errstr);
		 ...
		 const char *fmt, ...)
{
	...
	printk(KERN_CRIT "EXT3COW-fs (%s): error: %s: %pV\n",
	       sb->s_id, function, &vaf);
		  ...
		  const char *fmt, ...)
{
	...
	printk(KERN_WARNING "EXT3COW-fs (%s): warning: %s: %pV\n",
	       sb->s_id, function, &vaf);
 ...
 */
static struct block_device *ext3cow_blkdev_get(dev_t dev, struct super_block *sb)
{
...
fail:
	ext3cow_msg(sb, "error: failed to open journal device %s: %ld",
...
static void dump_orphan_list(struct super_block *sb, struct ext3cow_sb_info *sbi)
{
	...
	ext3cow_msg(sb, KERN_ERR, "error: sb orphan head is %d",
	       le32_to_cpu(sbi->s_es->s_last_orphan));

	ext3cow_msg(sb, KERN_ERR, "sb_info orphan list:");
	list_for_each(l, &sbi->s_orphan) {
		struct inode *inode = orphan_list_entry(l);
		ext3cow_msg(sb, KERN_ERR, "  "
...
static void ext3cow_put_super (struct super_block * sb)
{
	...
	sbi->s_journal = NULL;
	if (err < 0)
		ext3cow_abort(sb, __func__, "Couldn't clean up the journal");
...
static int ext3cow_drop_inode(struct inode *inode)
{
	int drop = generic_drop_inode(inode);

	trace_ext3cow_drop_inode(inode, drop);
	return drop;
}

static void ext3cow_i_callback(struct rcu_head *head)
{
...
static void ext3cow_destroy_inode(struct inode *inode)
{
	if (!list_empty(&(EXT3COW_I(inode)->i_orphan))) {
		printk("EXT3COW Inode %p: orphan list check failed!\n",
			EXT3COW_I(inode));
		print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 4,
				EXT3COW_I(inode), sizeof(struct ext3cow_inode_info),
				false);
		dump_stack();
	}
	call_rcu(&inode->i_rcu, ext3cow_i_callback);
}
...
{
	switch (mode) {
	case EXT3COW_MOUNT_JOURNAL_DATA:
		return "journal";
 ...
 */
static int ext3cow_show_options(struct seq_file *seq, struct dentry *root)
{
	...
	if (test_opt(sb, GRPID))
		seq_puts(seq, ",grpid");
	if (!test_opt(sb, GRPID) && (def_mount_opts & EXT3COW_DEFM_BSDGROUPS))
		seq_puts(seq, ",nogrpid");
	if (sbi->s_resuid != EXT3COW_DEF_RESUID ||
	    le16_to_cpu(es->s_def_resuid) != EXT3COW_DEF_RESUID) {
		seq_printf(seq, ",resuid=%u", sbi->s_resuid);
	}
	if (sbi->s_resgid != EXT3COW_DEF_RESGID ||
	    le16_to_cpu(es->s_def_resgid) != EXT3COW_DEF_RESGID) {
		seq_printf(seq, ",resgid=%u", sbi->s_resgid);
	}
	if (test_opt(sb, ERRORS_RO)) {
		int def_errors = le16_to_cpu(es->s_errors);

		if (def_errors == EXT3COW_ERRORS_PANIC ||
		    def_errors == EXT3COW_ERRORS_CONTINUE) {
	...
	if (!test_opt(sb, XATTR_USER) &&
	    (def_mount_opts & EXT3COW_DEFM_XATTR_USER)) {
	...
	if (test_opt(sb, POSIX_ACL))
		seq_puts(seq, ",acl");
	if (!test_opt(sb, POSIX_ACL) && (def_mount_opts & EXT3COW_DEFM_ACL))
		seq_puts(seq, ",noacl");
#endif
...
static struct inode *ext3cow_nfs_get_inode(struct super_block *sb,
...
static struct dentry *ext3cow_fh_to_dentry(struct super_block *sb, struct fid *fid,
		int fh_len, int fh_type)
{
	return generic_fh_to_dentry(sb, fid, fh_len, fh_type,
				    ext3cow_nfs_get_inode);
}

static struct dentry *ext3cow_fh_to_parent(struct super_block *sb, struct fid *fid,
		int fh_len, int fh_type)
{
...
static const struct dquot_operations ext3cow_quota_operations = {
...
static const struct quotactl_ops ext3cow_qctl_operations = {
...
static const struct super_operations ext3cow_sops = {
	...
	.drop_inode	= ext3cow_drop_inode,
	.evict_inode	= ext3cow_evict_inode,
	.put_super	= ext3cow_put_super,
	.sync_fs	= ext3cow_sync_fs,
	.freeze_fs	= ext3cow_freeze,
	.unfreeze_fs	= ext3cow_unfreeze,
...
static const struct export_operations ext3cow_export_ops = {
	.fh_to_dentry = ext3cow_fh_to_dentry,
	.fh_to_parent = ext3cow_fh_to_parent,
...
static ext3cow_fsblk_t get_sb_block(void **data, struct super_block *sb)
{
	...
	if (*options && *options != ',') {
		ext3cow_msg(sb, "error: invalid sb specification: %s",
		       (char *) *data);
		return 1;
	}
	if (*options == ',')
...
static int clear_qf_name(struct super_block *sb, int qtype) {
	...
	if (sb_any_quota_loaded(sb) &&
		sbi->s_qf_names[qtype]) {
		ext3cow_msg(sb, KERN_ERR, "Cannot change journaled quota options"
	...
	while ((p = strsep (&options, ",")) != NULL) {
		...
		case Opt_journal_update:
			...
			/* Eventually we will want to be able to create
			   ...
			   journal file. */
			if (is_remount) {
				ext3cow_msg(sb, KERN_ERR, "error: cannot specify "
					"journal on remount");
				return 0;
			}
...
set_qf_format:
			if (sb_any_quota_loaded(sb) &&
				...
				ext3cow_msg(sb, KERN_ERR, "error: cannot change "
	...
	if (sbi->s_qf_names[USRQUOTA] || sbi->s_qf_names[GRPQUOTA]) {
		...
		if (test_opt(sb, GRPQUOTA) || test_opt(sb, USRQUOTA)) {
			ext3cow_msg(sb, KERN_ERR, "error: old and new quota "
					"format mixing.");
			return 0;
		}

		if (!sbi->s_jquota_fmt) {
			ext3cow_msg(sb, KERN_ERR, "error: journaled quota format "
					"not specified.");
			return 0;
		}
	} else {
			    ...
			    int read_only)
{
	...
	if (!(sbi->s_mount_state & EXT3COW_VALID_FS))
		...
		/* @@@ We _will_ want to clear the valid bit if we find
                   ...
                   valid forever! :) */
	es->s_state &= cpu_to_le16(~EXT3COW_VALID_FS);
#endif
	...
	ext3cow_commit_super(sb, es, 1);
	if (test_opt(sb, DEBUG))
		ext3cow_msg(sb, KERN_INFO, "[bs=%lu, gc=%lu, "
	...
	if (EXT3COW_SB(sb)->s_journal->j_inode == NULL) {
		char b[BDEVNAME_SIZE];
		ext3cow_msg(sb, KERN_INFO, "using external journal on %s",
			bdevname(EXT3COW_SB(sb)->s_journal->j_dev, b));
	} else {
		ext3cow_msg(sb, KERN_INFO, "using internal journal");
	}
...
static int ext3cow_check_descriptors(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->s_groups_count; i++) {
		...
		ext3cow_fsblk_t first_block = ext3cow_group_first_block_no(sb, i);
				 ...
				 struct ext3cow_super_block * es)
{
	...
	if (bdev_read_only(sb->s_bdev)) {
		ext3cow_msg(sb, KERN_ERR, "error: write access "
			"unavailable, skipping orphan cleanup.");
		return;
	}

	/* Check if feature set allows readwrite operations */
	if (EXT3COW_HAS_RO_COMPAT_FEATURE(sb, ~EXT3COW_FEATURE_RO_COMPAT_SUPP)) {
		ext3cow_msg(sb, KERN_INFO, "Skipping orphan cleanup due to "
	...
	if (s_flags & MS_RDONLY) {
		ext3cow_msg(sb, KERN_INFO, "orphan cleanup on readonly fs");
		sb->s_flags &= ~MS_RDONLY;
	}
#ifdef CONFIG_QUOTA
	...
	while (es->s_last_orphan) {
		...
		inode = ext3cow_orphan_get(sb, le32_to_cpu(es->s_last_orphan));
		if (IS_ERR(inode)) {
			es->s_last_orphan = 0;
			break;
		}
...
#define PLURAL(x) (x), ((x)==1) ? "" : "s"
	...
	if (nr_orphans)
		ext3cow_msg(sb, KERN_INFO, "%d orphan inode%s deleted",
		       PLURAL(nr_orphans));
	if (nr_truncates)
		ext3cow_msg(sb, KERN_INFO, "%d truncate%s cleaned up",
		       PLURAL(nr_truncates));
#ifdef CONFIG_QUOTA
	/* Turn quotas off */
	for (i = 0; i < MAXQUOTAS; i++) {
...
static int ext3cow_fill_super (struct super_block *sb, void *data, int silent)
{
	...
	ext3cow_fsblk_t sb_block = get_sb_block(&data, sb);
	ext3cow_fsblk_t logic_sb_block;
	...
	if (!blocksize) {
		ext3cow_msg(sb, KERN_ERR, "error: unable to set blocksize");
		goto out_fail;
	}
	...
	if (!(bh = sb_bread(sb, logic_sb_block))) {
		ext3cow_msg(sb, KERN_ERR, "error: unable to read superblock");
		goto out_fail;
	}
	/*
	 ...
	 */
	es = (struct ext3cow_super_block *) (bh->b_data + offset);
	sbi->s_es = es;
	...
	if (le16_to_cpu(sbi->s_es->s_errors) == EXT3COW_ERRORS_PANIC)
		set_opt(sbi->s_mount_opt, ERRORS_PANIC);
	else if (le16_to_cpu(sbi->s_es->s_errors) == EXT3COW_ERRORS_CONTINUE)
		set_opt(sbi->s_mount_opt, ERRORS_CONT);
	else
		set_opt(sbi->s_mount_opt, ERRORS_RO);
	...
	if (EXT3COW_INODE_SIZE(sb) == 0 || EXT3COW_INODES_PER_GROUP(sb) == 0)
		goto cantfind_ext3cow;
	...
	sbi->s_groups_count = ((le32_to_cpu(es->s_blocks_count) -
			       le32_to_cpu(es->s_first_data_block) - 1)
				       / EXT3COW_BLOCKS_PER_GROUP(sb)) + 1;
	db_count = DIV_ROUND_UP(sbi->s_groups_count, EXT3COW_DESC_PER_BLOCK(sb));
	...
	if (err) {
		ext3cow_msg(sb, KERN_ERR, "error: insufficient memory");
		ret = err;
		goto failed_mount3;
	}
	...
	root = ext3cow_iget(sb, EXT3COW_ROOT_INO);
	if (IS_ERR(root)) {
		ext3cow_msg(sb, KERN_ERR, "error: get root inode failed");
		ret = PTR_ERR(root);
		goto failed_mount3;
	}
	if (!S_ISDIR(root->i_mode) || !root->i_blocks || !root->i_size) {
		iput(root);
		ext3cow_msg(sb, KERN_ERR, "error: corrupt root inode, run e2fsck");
		goto failed_mount3;
	}
	sb->s_root = d_make_root(root);
	if (!sb->s_root) {
		ext3cow_msg(sb, KERN_ERR, "error: get root dentry failed");
		ret = -ENOMEM;
		goto failed_mount3;
	}
	...
	if (needs_recovery) {
		...
		ext3cow_msg(sb, KERN_INFO, "recovery complete");
	}
	ext3cow_msg(sb, KERN_INFO, "mounted filesystem with %s data mode",
...
cantfind_ext3cow:
	if (!silent)
		ext3cow_msg(sb, KERN_INFO,
			"error: can't find ext3cow filesystem on dev %s.",
		       sb->s_id);
	goto failed_mount;
				   ...
				   unsigned int journal_inum)
{
	...
	journal_inode = ext3cow_iget(sb, journal_inum);
	if (IS_ERR(journal_inode)) {
		ext3cow_msg(sb, KERN_ERR, "error: no journal found");
		return NULL;
	}
	if (!journal_inode->i_nlink) {
		make_bad_inode(journal_inode);
		iput(journal_inode);
		ext3cow_msg(sb, KERN_ERR, "error: journal inode is deleted");
		return NULL;
	}
	...
	if (!S_ISREG(journal_inode->i_mode)) {
		ext3cow_msg(sb, KERN_ERR, "error: invalid journal inode");
		iput(journal_inode);
		return NULL;
	}
	...
	if (!journal) {
		ext3cow_msg(sb, KERN_ERR, "error: could not load journal inode");
		iput(journal_inode);
		return NULL;
	}
				       ...
				       dev_t j_dev)
{
	...
	bdev = ext3cow_blkdev_get(j_dev, sb);
	if (bdev == NULL)
		return NULL;
	...
	if (!(bh = __bread(bdev, sb_block, blocksize))) {
		ext3cow_msg(sb, KERN_ERR, "error: couldn't read superblock of "
			"external journal");
		goto out_bdev;
	}
	...
	if ((le16_to_cpu(es->s_magic) != EXT3COW_SUPER_MAGIC) ||
		...
		ext3cow_msg(sb, KERN_ERR, "error: external journal has "
			"bad superblock");
		brelse(bh);
		goto out_bdev;
	}
	...
	if (memcmp(EXT3COW_SB(sb)->s_es->s_journal_uuid, es->s_uuid, 16)) {
		ext3cow_msg(sb, KERN_ERR, "error: journal UUID does not match");
		brelse(bh);
		goto out_bdev;
	}
	...
	if (!bh_uptodate_or_lock(journal->j_sb_buffer)) {
		if (bh_submit_read(journal->j_sb_buffer)) {
			ext3cow_msg(sb, KERN_ERR, "I/O error on journal device");
			goto out_journal;
		}
	}
			     ...
			     unsigned long journal_devnum)
{
	...
	if (journal_devnum &&
		...
		ext3cow_msg(sb, KERN_INFO, "external journal device major/minor "
	...
	if (journal_inum && journal_dev) {
		ext3cow_msg(sb, KERN_ERR, "error: filesystem has both journal "
		       "and inode journals");
		return -EINVAL;
	}
	...
	if (!(journal->j_flags & JFS_BARRIER))
		printk(KERN_INFO "EXT3COW-fs: barriers not enabled\n");

	if (!really_read_only && test_opt(sb, UPDATE_JOURNAL)) {
		err = journal_update_format(journal);
		if (err)  {
			ext3cow_msg(sb, KERN_ERR, "error updating journal");
			journal_destroy(journal);
			return err;
		}
	}
	...
	if (err) {
		ext3cow_msg(sb, KERN_ERR, "error loading journal");
		journal_destroy(journal);
		return err;
	}
			       ...
			       unsigned int journal_inum)
{
	...
	journal = ext3cow_get_journal(sb, journal_inum);
	if (!journal)
		return -EINVAL;

	ext3cow_msg(sb, KERN_INFO, "creating new journal on inode %u",
	       journal_inum);

	err = journal_create(journal);
	if (err) {
		ext3cow_msg(sb, KERN_ERR, "error creating journal");
		journal_destroy(journal);
		return -EIO;
	}
			       ...
			       int sync)
{
	...
	if (buffer_write_io_error(sbh)) {
		 ...
		 */
		ext3cow_msg(sb, KERN_ERR, "previous I/O error to "
		       "superblock detected");
	...
	if (sync) {
		...
		if (buffer_write_io_error(sbh)) {
			ext3cow_msg(sb, KERN_ERR, "I/O error while writing "
			       "superblock");
			clear_buffer_write_io_error(sbh);
			set_buffer_uptodate(sbh);
		}
				   ...
				   struct ext3cow_super_block *es)
{
	...
	if (j_errno) {
		...
		ext3cow_warning(sb, __func__, "Filesystem error recorded "
			     "from previous mount: %s", errstr);
		ext3cow_warning(sb, __func__, "Marking fs in need of "
			     "filesystem check.");
...
static int ext3cow_freeze(struct super_block *sb)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		EXT3COW_CLEAR_INCOMPAT_FEATURE(sb, EXT3COW_FEATURE_INCOMPAT_RECOVER);
		error = ext3cow_commit_super(sb, EXT3COW_SB(sb)->s_es, 1);
		if (error)
			goto out;
	}
	return 0;
...
static int ext3cow_remount (struct super_block * sb, int * flags, char * data)
{
	...
	if (test_opt(sb, ABORT))
		ext3cow_abort(sb, __func__, "Abort forced by user");
	...
	if ((*flags & MS_RDONLY) != (sb->s_flags & MS_RDONLY) ||
		...
		if (*flags & MS_RDONLY) {
			 ...
			 */
			if (es->s_last_orphan) {
				ext3cow_msg(sb, KERN_WARNING, "warning: couldn't "
...
static int ext3cow_quota_on_mount(struct super_block *sb, int type)
{
	return dquot_quota_on_mount(sb, EXT3COW_SB(sb)->s_qf_names[type],
			 ...
			 struct path *path)
{
	...
	if (path->dentry->d_sb != sb)
		return -EXDEV;
	/* Journaling quota? */
	if (EXT3COW_SB(sb)->s_qf_names[type]) {
	 ...
	 */
	if (ext3cow_should_journal_data(path->dentry->d_inode)) {
		/*
		 * We don't need to lock updates but journal_flush() could
		 * otherwise be livelocked...
		 */
		journal_lock_updates(EXT3COW_SB(sb)->s_journal);
		err = journal_flush(EXT3COW_SB(sb)->s_journal);
		journal_unlock_updates(EXT3COW_SB(sb)->s_journal);
		if (err)
			return err;
	}
				...
				const char *data, size_t len, loff_t off)
{
	...
	if (sb->s_blocksize - offset < len) {
		ext3cow_msg(sb, KERN_WARNING, "Quota write (off=%llu, len=%llu)"
...
static struct dentry *ext3cow_mount(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data)
{
	return mount_bdev(fs_type, flags, dev_name, data, ext3cow_fill_super);
}
