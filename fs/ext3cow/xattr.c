...
static int ext3cow_xattr_list(struct dentry *dentry, char *buffer,
			   size_t buffer_size);

static struct mb_cache *ext3cow_xattr_cache;

static const struct xattr_handler *ext3cow_xattr_handler_map[] = {
...
const struct xattr_handler *ext3cow_xattr_handlers[] = {
...
{
	return ext3cow_xattr_list(dentry, buffer, size);
}
...
bad_block:	ext3cow_error(inode->i_sb, __func__,
		     ...
		     void *buffer, size_t buffer_size)
{
	...
	if (!ext3cow_test_inode_state(inode, EXT3COW_STATE_XATTR))
		return -ENODATA;
...
static int
ext3cow_xattr_list_entries(struct dentry *dentry, struct ext3cow_xattr_entry *entry,
...
static int
ext3cow_xattr_block_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	ext3cow_xattr_cache_insert(bh);
	error = ext3cow_xattr_list_entries(dentry, BFIRST(bh), buffer, buffer_size);
...
static int
ext3cow_xattr_ibody_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	struct inode *inode = dentry->d_inode;
	struct ext3cow_xattr_ibody_header *header;
	struct ext3cow_inode *raw_inode;
	...
	if (error)
		goto cleanup;
	error = ext3cow_xattr_list_entries(dentry, IFIRST(header),
					buffer, buffer_size);
 ...
 */
static int
ext3cow_xattr_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	int i_error, b_error;

	down_read(&EXT3COW_I(dentry->d_inode)->xattr_sem);
	i_error = ext3cow_xattr_ibody_list(dentry, buffer, buffer_size);
	if (i_error < 0) {
		b_error = 0;
	} else {
		if (buffer) {
		...
		b_error = ext3cow_xattr_block_list(dentry, buffer, buffer_size);
		if (b_error < 0)
			i_error = 0;
	}
	up_read(&EXT3COW_I(dentry->d_inode)->xattr_sem);
			 ...
			 struct buffer_head *bh)
{
	...
	error = ext3cow_journal_get_write_access(handle, bh);
	if (error)
		 goto out;
	...
	if (BHDR(bh)->h_refcount == cpu_to_le32(1)) {
		...
		error = ext3cow_journal_dirty_metadata(handle, bh);
		if (IS_SYNC(inode))
			handle->h_sync = 1;
...
inserted:
	if (!IS_LAST_ENTRY(s->first)) {
		...
		if (new_bh) {
			...
			ext3cow_fsblk_t goal = ext3cow_group_first_block_no(sb,
						EXT3COW_I(inode)->i_block_group);
			ext3cow_fsblk_t block;

			/*
			 * Protect us agaist concurrent allocations to the
			 * same inode from ext3cow_..._writepage(). Reservation
			 * code does not expect racing allocations.
			 */
			mutex_lock(&EXT3COW_I(inode)->truncate_mutex);
			block = ext3cow_new_block(handle, inode, goal, &error);
		     ...
		     struct ext3cow_xattr_ibody_find *is)
{
	...
	if (!IS_LAST_ENTRY(s->first)) {
		...
		ext3cow_clear_inode_state(inode, EXT3COW_STATE_XATTR);
	}
	return 0;
}
		      ...
		      int flags)
{
	...
	if (!value) {
		...
		if (!error && !bs.s.not_found) {
			...
			if (EXT3COW_I(inode)->i_file_acl && !bs.s.base) {
				error = ext3cow_xattr_block_find(inode, &i, &bs);
				if (error)
					goto cleanup;
			}
...
ext3cow_xattr_cache_insert(struct buffer_head *bh)
{
	...
	ce = mb_cache_entry_alloc(ext3cow_xattr_cache, GFP_NOFS);
	if (!ce) {
...
again:
	ce = mb_cache_entry_find_first(ext3cow_xattr_cache, inode->i_sb->s_bdev,
				       hash);
	while (ce) {
		struct buffer_head *bh;
...
{
	ext3cow_xattr_cache = mb_cache_create("ext3cow_xattr", 6);
