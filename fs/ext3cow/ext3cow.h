...
#define	EXT3COW_MAX_BLOCK_SIZE		65536
...
#define EXT3COW_FL_INHERITED (EXT3COW_SECRM_FL | EXT3COW_UNRM_FL | EXT3COW_COMPR_FL |\
			   EXT3COW_SYNC_FL | EXT3COW_NODUMP_FL |\
			   EXT3COW_NOATIME_FL | EXT3COW_COMPRBLK_FL |\
			   EXT3COW_NOCOMPR_FL | EXT3COW_JOURNAL_DATA_FL |\
			   EXT3COW_NOTAIL_FL | EXT3COW_DIRSYNC_FL)

/* Flags that are appropriate for regular files (all but dir-specific ones). */
#define EXT3COW_REG_FLMASK (~(EXT3COW_DIRSYNC_FL | EXT3COW_TOPDIR_FL))

/* Flags that are appropriate for non-directories/regular files. */
#define EXT3COW_OTHER_FLMASK (EXT3COW_NODUMP_FL | EXT3COW_NOATIME_FL)

/* Mask out flags that are inappropriate for the given type of inode. */
static inline __u32 ext3cow_mask_flags(umode_t mode, __u32 flags)
{
	if (S_ISDIR(mode))
		return flags;
	else if (S_ISREG(mode))
		return flags & EXT3COW_REG_FLMASK;
	else
		return flags & EXT3COW_OTHER_FLMASK;
}
...
/* EXT3COW_MOUNT_OLDALLOC was there */
...
#define EXT3COW_MOUNT_DATA_ERR_ABORT	0x400000 /* Abort on file data write
...
#define ext3cow_set_bit			__set_bit_le
#define ext3cow_set_bit_atomic		ext2_set_bit_atomic
#define ext3cow_clear_bit			__clear_bit_le
#define ext3cow_clear_bit_atomic		ext2_clear_bit_atomic
#define ext3cow_test_bit			test_bit_le
#define ext3cow_find_next_zero_bit		find_next_zero_bit_le
...
struct ext3cow_inode_info {
	 ...
	 */
	__u32	i_block_group;
	unsigned long	i_state_flags;	/* Dynamic state flags for ext3cow */
...
sb_bgl_lock(struct ext3cow_sb_info *sbi, unsigned int block_group)
{
 ...
 */
enum {
	EXT3COW_STATE_JDATA,		/* journaled data exists */
	EXT3COW_STATE_NEW,			/* inode is newly created */
	EXT3COW_STATE_XATTR,		/* has in-inode xattrs */
	EXT3COW_STATE_FLUSH_ON_CLOSE,	/* flush dirty pages on close */
};

static inline int ext3cow_test_inode_state(struct inode *inode, int bit)
{
	return test_bit(bit, &EXT3COW_I(inode)->i_state_flags);
}

static inline void ext3cow_set_inode_state(struct inode *inode, int bit)
{
	set_bit(bit, &EXT3COW_I(inode)->i_state_flags);
}

static inline void ext3cow_clear_inode_state(struct inode *inode, int bit)
{
	clear_bit(bit, &EXT3COW_I(inode)->i_state_flags);
}
...
#define EXT3COW_DIR_REC_LEN(name_len)     (((name_len) + 16 + EXT3COW_DIR_ROUND) & \
					 ~EXT3COW_DIR_ROUND)
#define EXT3COW_MAX_REC_LEN		((1<<16)-1)
 ...
 */
static inline unsigned ext3cow_rec_len_from_disk(__le16 dlen)
{
	unsigned len = le16_to_cpu(dlen);

#if (PAGE_CACHE_SIZE >= 65536)
	if (len == EXT3COW_MAX_REC_LEN)
		return 1 << 16;
#endif
	return len;
}

static inline __le16 ext3cow_rec_len_to_disk(unsigned len)
{
#if (PAGE_CACHE_SIZE >= 65536)
	if (len == (1 << 16))
		return cpu_to_le16(EXT3COW_MAX_REC_LEN);
	else if (len > (1 << 16))
		BUG();
#endif
...
extern int ext3cow_trim_fs(struct super_block *sb, struct fstrim_range *range);
...
/* fsync.c */
extern int ext3cow_sync_file(struct file *, loff_t, loff_t, int);
...
/* ialloc.c */
extern struct inode * ext3cow_new_inode (handle_t *, struct inode *,
...
extern struct inode *ext3cow_iget(struct super_block *, unsigned long);
extern int  ext3cow_write_inode (struct inode *, struct writeback_control *);
extern int  ext3cow_setattr (struct dentry *, struct iattr *);
extern void ext3cow_evict_inode (struct inode *);
extern int  ext3cow_sync_inode (handle_t *, struct inode *);
extern void ext3cow_discard_reservation (struct inode *);
extern void ext3cow_dirty_inode(struct inode *, int);
...
extern int ext3cow_can_truncate(struct inode *inode);
extern void ext3cow_truncate(struct inode *inode);
extern void ext3cow_set_inode_flags(struct inode *);
extern void ext3cow_get_inode_flags(struct ext3cow_inode_info *);
extern void ext3cow_set_aops(struct inode *inode);
extern int ext3cow_fiemap(struct inode *inode, struct fiemap_extent_info *fieinfo,
		       u64 start, u64 len);

/* ioctl.c */
extern long ext3cow_ioctl(struct file *, unsigned int, unsigned long);
...
/* super.c */
extern __printf(3, 4)
void ext3cow_error(struct super_block *, const char *, const char *, ...);
extern void __ext3cow_std_error (struct super_block *, const char *, int);
extern __printf(3, 4)
void ext3cow_abort(struct super_block *, const char *, const char *, ...);
extern __printf(3, 4)
void ext3cow_warning(struct super_block *, const char *, const char *, ...);
extern __printf(3, 4)
void ext3cow_msg(struct super_block *, const char *, const char *, ...);
...
do {								\
	if ((errno))						\
		__ext3cow_std_error((sb), __func__, (errno));	\
} while (0)
...
/* file.c */
extern const struct inode_operations ext3cow_file_inode_operations;
extern const struct file_operations ext3cow_file_operations;

/* namei.c */
extern const struct inode_operations ext3cow_dir_inode_operations;
extern const struct inode_operations ext3cow_special_inode_operations;

/* symlink.c */
extern const struct inode_operations ext3cow_symlink_inode_operations;
extern const struct inode_operations ext3cow_fast_symlink_inode_operations;
...
#define EXT3COW_DATA_TRANS_BLOCKS(sb)	(EXT3COW_SINGLEDATA_TRANS_BLOCKS + \
					 ...
					 EXT3COW_MAXQUOTAS_TRANS_BLOCKS(sb))

/* Delete operations potentially hit one directory's namespace plus an
 * entire inode, plus arbitrary amounts of bitmap/indirection data.  Be
 * generous.  We can grow the delete transaction later if necessary. */

#define EXT3COW_DELETE_TRANS_BLOCKS(sb)   (EXT3COW_MAXQUOTAS_TRANS_BLOCKS(sb) + 64)
...
#endif
#define EXT3COW_MAXQUOTAS_TRANS_BLOCKS(sb) (MAXQUOTAS*EXT3COW_QUOTA_TRANS_BLOCKS(sb))
#define EXT3COW_MAXQUOTAS_INIT_BLOCKS(sb) (MAXQUOTAS*EXT3COW_QUOTA_INIT_BLOCKS(sb))
#define EXT3COW_MAXQUOTAS_DEL_BLOCKS(sb) (MAXQUOTAS*EXT3COW_QUOTA_DEL_BLOCKS(sb))
...
#define ext3cow_journal_get_undo_access(handle, bh) \
	__ext3cow_journal_get_undo_access(__func__, (handle), (bh))
#define ext3cow_journal_get_write_access(handle, bh) \
	__ext3cow_journal_get_write_access(__func__, (handle), (bh))
#define ext3cow_journal_revoke(handle, blocknr, bh) \
	__ext3cow_journal_revoke(__func__, (handle), (blocknr), (bh))
#define ext3cow_journal_get_create_access(handle, bh) \
	__ext3cow_journal_get_create_access(__func__, (handle), (bh))
#define ext3cow_journal_dirty_metadata(handle, bh) \
	__ext3cow_journal_dirty_metadata(__func__, (handle), (bh))
#define ext3cow_journal_forget(handle, bh) \
	__ext3cow_journal_forget(__func__, (handle), (bh))
...
#define ext3cow_journal_stop(handle) \
	__ext3cow_journal_stop(__func__, (handle))
...
#include <trace/events/ext3cow.h>
