...
static int ext3cow_block_truncate_page(struct inode *inode, loff_t from);
			...
			struct buffer_head *bh, ext3cow_fsblk_t blocknr)
{
	...
	trace_ext3cow_forget(inode, is_metadata, blocknr);
	...
	if (err)
		ext3cow_abort(inode->i_sb, __func__,
...
static int truncate_restart_transaction(handle_t *handle, struct inode *inode)
{
	...
	/*
	 * Drop truncate_mutex to avoid deadlock with ext3cow_get_blocks_handle
	 * At this moment, get_block can be called only for blocks inside
	 * i_size since page cache has been already dropped and writes are
	 * blocked by i_mutex. So we can safely drop the truncate_mutex.
	 */
	mutex_unlock(&EXT3COW_I(inode)->truncate_mutex);
	ret = ext3cow_journal_restart(handle, blocks_for_truncate(inode));
 ...
 */
void ext3cow_evict_inode (struct inode *inode)
{
	struct ext3cow_inode_info *ei = EXT3COW_I(inode);
	struct ext3cow_block_alloc_info *rsv;
	handle_t *handle;
	int want_delete = 0;

	trace_ext3cow_evict_inode(inode);
	 ...
	 * use page cache.
	 *
	 * The s_journal check handles the case when ext3cow_get_journal() fails
	 * and puts the journal inode.
	 */
	if (inode->i_nlink && ext3cow_should_journal_data(inode) &&
	    EXT3COW_SB(inode->i_sb)->s_journal &&
	    (S_ISLNK(inode->i_mode) || S_ISREG(inode->i_mode))) {
		tid_t commit_tid = atomic_read(&ei->i_datasync_tid);
		journal_t *journal = EXT3COW_SB(inode->i_sb)->s_journal;
	 ...
	 * link.  Note that ext3cow_orphan_del() has to be able to cope with the
	 * deletion of a non-existent orphan - ext3cow_truncate() could
			...
			long i_block, int offsets[4], int *boundary)
{
	...
	//int ptrs_bits = EXT3COW_ADDR_PER_BLOCK_BITS(inode->i_sb);
...
/**
 *	ext3cow_find_goal - find a preferred place for allocation.
 *	@inode: owner
 *	@block:  block we want
			...
			long block, Indirect *where, int num, int blks)
{
	...
	*where->p = where->key;
	...
	/* ext3cow_mark_inode_dirty already updated i_sync_tid */
		...
		int create)
{
	...
	trace_ext3cow_get_blocks_enter(inode, iblock, maxblocks, create);
	...
	*/
	...
	goal = ext3cow_find_goal(inode, iblock, partial);
...
out:
	trace_ext3cow_get_blocks_exit(inode, iblock,
			...
			struct buffer_head *bh_result, int create)
{
	...
	if (create && !handle) {	/* Direct IO write... */
		...
		handle = ext3cow_journal_start(inode, DIO_CREDITS +
				EXT3COW_MAXQUOTAS_TRANS_BLOCKS(inode->i_sb));
		if (IS_ERR(handle)) {
			ret = PTR_ERR(handle);
			goto out;
		}
...
int ext3cow_fiemap(struct inode *inode, struct fiemap_extent_info *fieinfo,
		u64 start, u64 len)
{
					...
					struct buffer_head *bh)
{
	...
	ret = ext3cow_journal_get_write_access(handle, bh);
	if (!ret && dirty)
		ret = ext3cow_journal_dirty_metadata(handle, bh);
	return ret;
}
 ...
 */
static void ext3cow_truncate_failed_write(struct inode *inode)
{
 ...
 */
static void ext3cow_truncate_failed_direct_write(struct inode *inode)
{
	ext3cow_block_truncate_page(inode, inode->i_size);
	ext3cow_truncate(inode);
}

static int ext3cow_write_begin(struct file *file, struct address_space *mapping,
				...
				struct page **pagep, void **fsdata)
{
	...
	int needed_blocks = ext3cow_writepage_trans_blocks(inode) + 1;

	trace_ext3cow_write_begin(inode, pos, len, flags);
...
/* ADWAIT: TODO: Find a place for this.
	if(page_has_buffers(page))
	ret = walk_page_buffers(handle, page_buffers(page),
                            from, to, NULL, ext3cow_clear_buffer_mapped);
*/
	ret = __block_write_begin(page, pos, len, ext3cow_get_block);
	if (ret)
...
write_begin_failed:
	if (ret) {
		 ...
		 * finishes. Do this only if ext3cow_can_truncate() agrees so
		 * that orphan processing code is happy.
		 */
		if (pos + len > inode->i_size && ext3cow_can_truncate(inode))
			ext3cow_orphan_add(handle, inode);
		ext3cow_journal_stop(handle);
		unlock_page(page);
		page_cache_release(page);
		if (pos + len > inode->i_size)
			ext3cow_truncate_failed_write(inode);
	}
...
int ext3cow_journal_dirty_data(handle_t *handle, struct buffer_head *bh)
{
	...
	if (err)
		ext3cow_journal_abort_handle(__func__, __func__,
						bh, handle, err);
	return err;
}
...
/*
 * This is nasty and subtle: ext3cow_write_begin() could have allocated blocks
...
static void update_file_sizes(struct inode *inode, loff_t pos, unsigned copied)
{
	...
	if (pos + copied > EXT3COW_I(inode)->i_disksize) {
		EXT3COW_I(inode)->i_disksize = pos + copied;
 ...
 */
static int ext3cow_ordered_write_end(struct file *file,
				...
				struct page *page, void *fsdata)
{
	...
	trace_ext3cow_ordered_write_end(inode, pos, len, copied);
...
static int ext3cow_writeback_write_end(struct file *file,
				struct address_space *mapping,
				loff_t pos, unsigned len, unsigned copied,
				struct page *page, void *fsdata)
{
	...
	trace_ext3cow_writeback_write_end(inode, pos, len, copied);
	copied = block_write_end(file, mapping, pos, len, copied, page, fsdata);
	update_file_sizes(inode, pos, copied);
	/*
	 * There may be allocated blocks outside of i_size because
	...
	if (pos + len > inode->i_size && ext3cow_can_truncate(inode))
		ext3cow_orphan_add(handle, inode);
	ret = ext3cow_journal_stop(handle);
	unlock_page(page);
	page_cache_release(page);
...
static int ext3cow_journalled_write_end(struct file *file,
				struct address_space *mapping,
				loff_t pos, unsigned len, unsigned copied,
				struct page *page, void *fsdata)
{
	...
	trace_ext3cow_journalled_write_end(inode, pos, len, copied);
	from = pos & (PAGE_CACHE_SIZE - 1);
	to = from + len;
	...
	if (pos + len > inode->i_size && ext3cow_can_truncate(inode))
		ext3cow_orphan_add(handle, inode);
	ext3cow_set_inode_state(inode, EXT3COW_STATE_JDATA);
	atomic_set(&ei->i_datasync_tid, handle->h_transaction->t_tid);
...
static sector_t ext3cow_bmap(struct address_space *mapping, sector_t block)
{
	...
	if (ext3cow_test_inode_state(inode, EXT3COW_STATE_JDATA)) {
		/*
		...
		ext3cow_clear_inode_state(inode, EXT3COW_STATE_JDATA);
				...
				struct writeback_control *wbc)
{
	...
	WARN_ON_ONCE(IS_RDONLY(inode) &&
		     !(EXT3COW_SB(inode->i_sb)->s_mount_state & EXT3COW_ERROR_FS));
	...
	trace_ext3cow_ordered_writepage(page);
	if (!page_has_buffers(page)) {
				...
				struct writeback_control *wbc)
{
	...
	trace_ext3cow_writeback_writepage(page);
	if (page_has_buffers(page)) {
				...
				struct writeback_control *wbc)
{
	...
	trace_ext3cow_journalled_writepage(page);
	handle = ext3cow_journal_start(inode, ext3cow_writepage_trans_blocks(inode));
	if (IS_ERR(handle)) {
		ret = PTR_ERR(handle);
		goto no_write;
	...
	if (!page_has_buffers(page) || PageChecked(page)) {
		...
		if (ret == 0)
			ret = err;
		ext3cow_set_inode_state(inode, EXT3COW_STATE_JDATA);
		atomic_set(&EXT3COW_I(inode)->i_datasync_tid,
...
static void ext3cow_invalidatepage(struct page *page, unsigned long offset)
{
	...
	trace_ext3cow_invalidatepage(page, offset);
...
static int ext3cow_releasepage(struct page *page, gfp_t wait)
{
	...
	trace_ext3cow_releasepage(page);
			...
			unsigned long nr_segs)
{
	...
	trace_ext3cow_direct_IO_enter(inode, offset, iov_length(iov, nr_segs), rw);
...
retry:
	...
	if (unlikely((rw & WRITE) && ret < 0)) {
		...
		if (end > isize)
			ext3cow_truncate_failed_direct_write(inode);
	}
	if (ret == -ENOSPC && ext3cow_should_retry_alloc(inode->i_sb, &retries))
		goto retry;
...
out:
	trace_ext3cow_direct_IO_exit(inode, offset,
...
static const struct address_space_operations ext3cow_ordered_aops = {
	...
	.write_begin		= ext3cow_write_begin,
	.write_end		= ext3cow_ordered_write_end,
	.bmap			= ext3cow_bmap,
...
static const struct address_space_operations ext3cow_writeback_aops = {
	...
	.write_begin		= ext3cow_write_begin,
	.write_end		= ext3cow_writeback_write_end,
	.bmap			= ext3cow_bmap,
	.invalidatepage		= ext3cow_invalidatepage,
	.releasepage		= ext3cow_releasepage,
	.direct_IO		= ext3cow_direct_IO,
...
static const struct address_space_operations ext3cow_journalled_aops = {
	...
	.write_begin		= ext3cow_write_begin,
	.write_end		= ext3cow_journalled_write_end,
...
static int ext3cow_block_truncate_page(struct inode *inode, loff_t from)
{
	...
	if (!ext3cow_should_writeback_data(inode)) {
		...
		unsigned long count, __le32 *first, __le32 *last)
{
	...
	if (try_to_extend_transaction(handle, inode)) {
		if (bh) {
			BUFFER_TRACE(bh, "call ext3cow_journal_dirty_metadata");
			if (ext3cow_journal_dirty_metadata(handle, bh))
				return;
		}
		ext3cow_mark_inode_dirty(handle, inode);
		...
		if (bh) {
			...
			if (ext3cow_journal_get_write_access(handle, bh))
				return;
		}
	}
			   ...
			   __le32 *first, __le32 *last)
{
	...
	if (this_bh) {
		...
		if (bh2jh(this_bh))
			...
			ext3cow_error(inode->i_sb, "ext3cow_free_data",
...
int ext3cow_can_truncate(struct inode *inode)
{
	if (S_ISREG(inode->i_mode))
		return 1;
	if (S_ISDIR(inode->i_mode))
		return 1;
	if(EXT3COW_IS_UNCHANGEABLE(inode)) /* znjp */
                return 0;
	if (S_ISLNK(inode->i_mode))
		return !ext3cow_inode_is_fast_symlink(inode);
	return 0;
}
...
void ext3cow_truncate(struct inode *inode)
{
	...
	trace_ext3cow_truncate_enter(inode);

	if (!ext3cow_can_truncate(inode))
		goto out_notrans;

	if (inode->i_size == 0 && ext3cow_should_writeback_data(inode))
		ext3cow_set_inode_state(inode, EXT3COW_STATE_FLUSH_ON_CLOSE);
...
out_stop:
	 ...
	 * ext3cow_evict_inode(), and we allow that function to clean up the
	 * orphan info for us.
	 */
	if (inode->i_nlink)
		ext3cow_orphan_del(handle, inode);
	...
	ext3cow_journal_stop(handle);
	trace_ext3cow_truncate_exit(inode);
	return;
out_notrans:
	/*
...
ext3cow_fsblk_t ext3cow_get_inode_block(struct super_block *sb,
		unsigned long ino, struct ext3cow_iloc *iloc)
{
	...
	gdp = ext3cow_get_group_desc(sb, block_group, NULL);
	if (!gdp)
		return 0;
	/*
...
make_io:
		 ...
		 */
		trace_ext3cow_load_inode(inode);
		get_bh(bh);
		bh->b_end_io = end_buffer_read_sync;
		submit_bh(READ | REQ_META | REQ_PRIO, bh);
		wait_on_buffer(bh);
...
int ext3cow_get_inode_loc(struct inode *inode, struct ext3cow_iloc *iloc)
{
	...
	return __ext3cow_get_inode_loc(inode, iloc,
		!ext3cow_test_inode_state(inode, EXT3COW_STATE_XATTR));
}
...
/* Propagate flags from i_flags to EXT3COW_I(inode)->i_flags */
void ext3cow_get_inode_flags(struct ext3cow_inode_info *ei)
{
	unsigned int flags = ei->vfs_inode.i_flags;

	ei->i_flags &= ~(EXT3COW_SYNC_FL|EXT3COW_APPEND_FL|
			EXT3COW_IMMUTABLE_FL|EXT3COW_NOATIME_FL|EXT3COW_DIRSYNC_FL);
	if (flags & S_SYNC)
		ei->i_flags |= EXT3COW_SYNC_FL;
	if (flags & S_APPEND)
		ei->i_flags |= EXT3COW_APPEND_FL;
	if (flags & S_IMMUTABLE)
		ei->i_flags |= EXT3COW_IMMUTABLE_FL;
	if (flags & S_NOATIME)
		ei->i_flags |= EXT3COW_NOATIME_FL;
	if (flags & S_DIRSYNC)
		ei->i_flags |= EXT3COW_DIRSYNC_FL;
}

struct inode *ext3cow_iget(struct super_block *sb, unsigned long ino)
{
	...
	ret = __ext3cow_get_inode_loc(inode, &iloc, 0);
	if (ret < 0)
		goto bad_inode;
	bh = iloc.bh;
	...
	/* For versioning -znjp */
	/* Adwait: this function replaces ext3cow_read_inode in the original patch */
	...
	if (inode->i_ino >= EXT3COW_FIRST_INO(inode->i_sb) + 1 &&
		...
		if (ei->i_extra_isize == 0) {
			...
			if (*magic == cpu_to_le32(EXT3COW_XATTR_MAGIC))
				 ext3cow_set_inode_state(inode, EXT3COW_STATE_XATTR);
		}
	} else
		ei->i_extra_isize = 0;
...
again:
	...
	if (ext3cow_test_inode_state(inode, EXT3COW_STATE_NEW))
 ...
 */
	...
	if (!err)
		err = rc;
	ext3cow_clear_inode_state(inode, EXT3COW_STATE_NEW);
 ...
 */
int ext3cow_write_inode(struct inode *inode, struct writeback_control *wbc)
{
...
int ext3cow_setattr(struct dentry *dentry, struct iattr *attr)
{
	...
	if ((ia_valid & ATTR_UID && attr->ia_uid != inode->i_uid) ||
		...
		handle = ext3cow_journal_start(inode, EXT3COW_MAXQUOTAS_INIT_BLOCKS(inode->i_sb)+
					EXT3COW_MAXQUOTAS_DEL_BLOCKS(inode->i_sb)+3);
		if (IS_ERR(handle)) {
	...
	if (S_ISREG(inode->i_mode) &&
		...
		rc = ext3cow_block_truncate_page(inode, attr->ia_size);
		if (rc) {
...
static int ext3cow_writepage_trans_blocks(struct inode *inode)
{
	...
	ret += EXT3COW_MAXQUOTAS_TRANS_BLOCKS(inode->i_sb);
#endif
...
int ext3cow_mark_inode_dirty(handle_t *handle, struct inode *inode)
{
	...
	might_sleep();
	trace_ext3cow_mark_inode_dirty(inode, _RET_IP_);
 ...
 */
void ext3cow_dirty_inode(struct inode *inode, int flags)
{
