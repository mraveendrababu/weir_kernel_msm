			      ...
			      struct ext3cow_new_group_data *input)
{
	...
	if (group != sbi->s_groups_count)
		...
		ext3cow_warning(sb, __func__, "Last group not full");
	else if (input->reserved_blocks > input->blocks_count / 5)
		ext3cow_warning(sb, __func__, "Reserved blocks too high (%u)",
			     input->reserved_blocks);
	else if (free_blocks_count < 0)
		ext3cow_warning(sb, __func__, "Bad blocks count %u",
			     input->blocks_count);
...
/*
 * If we have fewer than thresh credits, extend by EXT3COW_MAX_TRANS_DATA.
					 ...
					 struct buffer_head *bh)
{
	...
	err = ext3cow_journal_extend(handle, EXT3COW_MAX_TRANS_DATA);
	if (err < 0)
		return err;
	if (err) {
				  ...
				  struct ext3cow_new_group_data *input)
{
	...
	for (i = 0, bit = 1, block = start + 1;
		...
		unlock_buffer(gdb);
		err = ext3cow_journal_dirty_metadata(handle, gdb);
		if (err) {
			brelse(gdb);
			goto exit_bh;
		}
	...
	for (i = 0, block = input->inode_table, bit = block - start;
		...
		err = ext3cow_journal_dirty_metadata(handle, it);
		if (err) {
			brelse(it);
			goto exit_bh;
		}
		       ...
		       struct buffer_head **primary)
{
	...
	*primary = sb_bread(sb, gdblock);
	...
	err = ext3cow_journal_dirty_metadata(handle, dind);
	if (err)
	...
	err = ext3cow_journal_dirty_metadata(handle, *primary);
	if (err)
		goto exit_group_desc;
...
int ext3cow_group_add(struct super_block *sb, struct ext3cow_new_group_data *input)
{
	...
	if (le32_to_cpu(es->s_blocks_count) + input->blocks_count <
		...
		ext3cow_warning(sb, __func__, "blocks_count overflow\n");
		return -EINVAL;
	}

	if (le32_to_cpu(es->s_inodes_count) + EXT3COW_INODES_PER_GROUP(sb) <
	    le32_to_cpu(es->s_inodes_count)) {
		ext3cow_warning(sb, __func__, "inodes_count overflow\n");
		return -EINVAL;
	}
	...
	if (reserved_gdb || gdb_off == 0) {
		...
		inode = ext3cow_iget(sb, EXT3COW_RESIZE_INO);
		if (IS_ERR(inode)) {
			ext3cow_warning(sb, __func__,
	...
	le32_add_cpu(&es->s_inodes_count, EXT3COW_INODES_PER_GROUP(sb));
		      ...
		      ext3cow_fsblk_t n_blocks_count)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "EXT3COW-fs: extending last group from "E3FSBLK
	...
	if (IS_ERR(handle)) {
		err = PTR_ERR(handle);
		ext3cow_warning(sb, __func__, "error %d on journal start",err);
		goto exit_put;
	}

	mutex_lock(&EXT3COW_SB(sb)->s_resize_lock);
	if (o_blocks_count != le32_to_cpu(es->s_blocks_count)) {
		ext3cow_warning(sb, __func__,
			     "multiple resizers run on filesystem!");
		mutex_unlock(&EXT3COW_SB(sb)->s_resize_lock);
	...
	ext3cow_debug("freeing blocks "E3FSBLK" through "E3FSBLK"\n",
