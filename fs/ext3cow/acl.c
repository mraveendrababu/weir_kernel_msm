...
ext3cow_acl_to_disk(const struct posix_acl *acl, size_t *size)
{
	...
	*size = ext3cow_acl_size(acl->a_count);
	ext_acl = kmalloc(sizeof(ext3cow_acl_header) + acl->a_count *
			sizeof(ext3cow_acl_entry), GFP_NOFS);
	if (!ext_acl)
		return ERR_PTR(-ENOMEM);
...
ext3cow_init_acl(handle_t *handle, struct inode *inode, struct inode *dir)
{
	...
	if (test_opt(inode->i_sb, POSIX_ACL) && acl) {
		...
		if (error > 0) {
			...
			error = ext3cow_set_acl(handle, inode, ACL_TYPE_ACCESS, acl);
		}
	}
cleanup:
	posix_acl_release(acl);
 ...
 */
static size_t
ext3cow_xattr_list_acl_access(struct dentry *dentry, char *list, size_t list_len,
...
static size_t
ext3cow_xattr_list_acl_default(struct dentry *dentry, char *list, size_t list_len,
			    const char *name, size_t name_len, int type)
{
...
static int
ext3cow_xattr_get_acl(struct dentry *dentry, const char *name, void *buffer,
		   size_t size, int type)
{
	...
	acl = ext3cow_get_acl(dentry->d_inode, type);
	if (IS_ERR(acl))
		return PTR_ERR(acl);
	if (acl == NULL)
		return -ENODATA;
...
static int
ext3cow_xattr_set_acl(struct dentry *dentry, const char *name, const void *value,
...
const struct xattr_handler ext3cow_xattr_acl_access_handler = {
	.prefix	= POSIX_ACL_XATTR_ACCESS,
	.flags	= ACL_TYPE_ACCESS,
	.list	= ext3cow_xattr_list_acl_access,
	.get	= ext3cow_xattr_get_acl,
	.set	= ext3cow_xattr_set_acl,
};

const struct xattr_handler ext3cow_xattr_acl_default_handler = {
