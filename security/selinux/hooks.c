...
static void inode_free_security(struct inode *inode)
{
	...
	struct superblock_security_struct *sbsec = lsm_get_super(inode->i_sb, &selinux_ops);
...
{
	struct superblock_security_struct *sbsec = lsm_get_super(sb, &selinux_ops);
				...
				struct security_mnt_opts *opts)
{
	...
	struct inode_security_struct *root_isec = lsm_get_inode(inode, &selinux_ops);
...
{
	const struct superblock_security_struct *oldsbsec = lsm_get_super(oldsb, &selinux_ops);
	struct superblock_security_struct *newsbsec = lsm_get_super(newsb, &selinux_ops);
	...
	if (set_context) {
		...
		if (!set_rootcontext) {
			...
			struct inode_security_struct *newisec = lsm_get_inode(newinode, &selinux_ops);
			newisec->sid = sid;
		}
	...
	if (new_tsec->sid == old_tsec->sid) {
		...
		if (bprm->unsafe &
			...
			if (likely(tracer != NULL)) {
				sec = lsm_get_cred(__task_cred(tracer),&selinux_ops);
				ptsid = sec->sid;
			}
			rcu_read_unlock();
...
static int selinux_sb_remount(struct super_block *sb, void *data)
{
	...
	for (i = 0; i < opts.num_mnt_opts; i++) {
		...
		case ROOTCONTEXT_MNT: {
			...
			root_isec = lsm_get_inode(sb->s_root->d_inode,&selinux_ops);
...
static int selinux_cred_alloc_blank(struct cred *cred, gfp_t gfp)
{
	...
	printk("in SELINUX alloc sec for pid %d\n", current->pid);
...
{
	const struct task_security_struct *tsec = lsm_get_cred(current_cred(), &selinux_ops);
	struct inode_security_struct *isec = lsm_get_inode(SOCK_INODE(sock), &selinux_ops);
...
static int selinux_socket_unix_stream_connect(struct sock *sock,
					      ...
					      struct sock *newsk)
{
	struct sk_security_struct *sksec_sock = lsm_get_sock(sock, &selinux_ops);
	struct sk_security_struct *sksec_other = lsm_get_sock(other, &selinux_ops);
	struct sk_security_struct *sksec_new = lsm_get_sock(newsk, &selinux_ops);
	struct common_audit_data ad;
	struct selinux_audit_data sad = {0,};
	struct lsm_network_audit net = {0,};
	int err;
...
{
	struct inode_security_struct *isec = lsm_get_inode(SOCK_INODE(parent), &selinux_ops);
	struct sk_security_struct *sksec = lsm_get_sock(sk, &selinux_ops);
				      ...
				      u16 family)
{
	...
	if (skb->sk) {
		struct sk_security_struct *sksec = lsm_get_sock(skb->sk, &selinux_ops);
		sid = sksec->sid;
	} else
...
static __init int selinux_init(void)
{
	...
	sel_inode_cache = kmem_cache_create("selinux_inode_security",
					    sizeof(struct inode_security_struct),0, SLAB_PANIC, NULL);
	avc_init();
